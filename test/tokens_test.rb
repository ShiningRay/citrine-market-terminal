...
# 设计令牌守卫：令牌唯一来源是 app/tokens.rb（浏览器 CSS / 浏览器视图 / 共享逻辑
# Format / 原生 theme.rb 四处共用），这里把"唯一"锁死——
#
...
#   4. Format 的涨跌色必须派生自令牌（红涨绿跌只定义一次）；
#   5. 原生 theme.rb 只准保留 GRID 一个本地色值（原生独有的图表网格色）。
#
...
class TokensTest < Minitest::Test
  ...
  # 不进 CSS 的令牌：无（sma 由 .sma-dot 引用，其余都是 :root 原有变量）
  NOT_IN_CSS = [].freeze
  ...
  def test_every_css_var_reference_has_a_token
    missing = css_var_refs - Market::Tokens::MAP.keys
    assert_empty missing, "styles.css 引用了未定义的令牌：#{missing.inspect}"
  end

  def test_every_token_is_used_or_whitelisted
    orphaned = Market::Tokens::MAP.keys - css_var_refs - NOT_IN_CSS
    assert_empty orphaned,
                 "这些令牌没有任何使用方（styles.css 未引用、也不在白名单）：#{orphaned.inspect}"
  end

  def test_format_colors_derive_from_tokens
    format_source = read("app/format.rb")
    assert_includes format_source, "UP_COLOR = Tokens[:up]",
                    "Format::UP_COLOR 必须派生自令牌（红涨绿跌只定义一次）"
    assert_includes format_source, "DOWN_COLOR = Tokens[:down]",
                    "Format::DOWN_COLOR 必须派生自令牌"
    assert_equal Market::Tokens[:up], Market::Format::UP_COLOR
    assert_equal Market::Tokens[:down], Market::Format::DOWN_COLOR
  end
  ...
  def test_native_theme_derives_from_tokens_without_local_colors
    ...
    assert_equal ["#16233c"], locals,
                 "native/theme.rb 里出现了新的字面量色值——它只允许出现在 GRID（原生独有的图表网格色）"
  end
end
